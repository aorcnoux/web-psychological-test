...
const Question = () => {
  ...
  const { answers, setAnswer } = useTest();

  const questionId = parseInt(id || "1", 10);
  const question = questions[questionId - 1];

  const [selectedOption, setSelectedOption] = useState<number | null>(
    answers[questionId - 1] ?? null
  );

  useEffect(() => {
    setSelectedOption(answers[questionId - 1] ?? null);
  }, [questionId, answers]);

  const handleNext = () => {
    if (selectedOption !== null) {
      setAnswer(questionId - 1, selectedOption);
  ...
  return (
    ...
    style={{ backgroundImage: "url(/qbg.jpeg)" }} // 改成你的圖片路徑
  >  
